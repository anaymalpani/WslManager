...
namespace WslManager.Extensions
{
    ...
    /// </summary>
    public sealed class NamesGenerator
	{
		private static readonly string[] left = new string[] {
			"admiring",
			"adoring",
			"affectionate",
			"agitated",
		...
		private static readonly string[] right = new string[] {
		...
		public string GetRandomName(bool retry)
		{
			...
			string name = String.Format(culture, "{0}_{1}",
				left[rnd.Next(left.Length)],
				right[rnd.Next(right.Length)]);
			if (retry)
			{
